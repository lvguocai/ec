...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
        ...
        /* 查询规格 */
        if (trim($arr[$key]['goods_attr']) != '')
        {
        	 
        	$arr[$key]['goods_attr']=addslashes($arr[$key]['goods_attr']);
        
        
        
        
        	$hello =  explode(" ",$arr[$key]['goods_attr']);
        
        	for($index=0;$index<count($hello);$index++)
        	{
        	//echo $hello[$index];echo "</br>";
        	$hellob =  explode(":",$hello[$index]);
        	//print_r($hellob[1]);
        	if(!empty($hellob[1])){
        	$arr[$key]['kuan'][$index] = $hellob[1];
        	}
        	}
        	if(!empty($arr[$key]['kuan'][1])){
        	$arr[$key]['kuan0'] = $arr[$key]['kuan'][0];
        		$arr[$key]['kuan1'] = $arr[$key]['kuan'][1];
        	}
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {	
            ...
            //print_r($row['goods_attr']);
            
            
            $hello =  explode(" ",$row['goods_attr']);
			...
			{ 
				...
				//print_r($hellob[1]);
				if(!empty($hellob[1])){
					$row['kuan'][$index] = $hellob[1];
				}
			} 
			if(!empty($row['kuan'][1])){
	            $row['kuan0'] = $row['kuan'][0];
	            $row['kuan1'] = $row['kuan'][1];
			}
